...
const FIELDS_TO_TRUST = {
    [TABLES.VILLAS]: [ 'description', 'additional_info' ]
}
...
app.controller('villaCtrl', ['$scope','$http', 'setCookieFact', '$sce', function($s, $h,  setCookieFact, $sce){
    ...
    function initVilla(villa) {
        FIELDS_TO_TRUST[TABLES.VILLAS].forEach(fieldName => {
            villa[fieldName] = $sce.trustAsHtml(villa[fieldName]);
        });
        $s.villaInfo = villa;
        $s.fieldValueId = villa.field_value_id;

        $h.get(`/api/data/table?t=${TABLES.ADDITIONAL_FIELDS}&where[f1]=${villa.field_value_id}`).success(res => initAdditionalFields(res[0]))
        $h.get(`/api/data/table?t=${TABLES.IMAGES}&where[f1]=${villa.field_value_id}`).success(res => initSliderImages(res));

        villa.f12 = villa.f12 || [];
        villa.f12.forEach(benefitRow => {
            benefits.forEach(function(benefit){
                if(benefitRow.benefit_name == benefit.name) {
    ...
    function initAdditionalFields(fields) {
        let villa = $s.villaInfo;
        villa.addFields = fields || {};
        if (fields.f8) {
            const [ latitude, longitude ] = fields.f8.split(',').map(val => val.trim);
            const title = $s.targetDirect || 'No direction';
            $s.coordinates.push({ latitude, longitude, title });
    ...
    function initSliderImages(imageRows) {
        let { villa, slides, alts } = $s;
        if (imageRows) imageRows.forEach(imageRow => {
